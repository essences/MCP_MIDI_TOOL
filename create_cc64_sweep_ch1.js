...
// CC64スイープ: Ch1で 0→127→0 と昇降する連続イベント (純JS版)
const events = [];

// Delta 0: Tempo 120bpm
events.push(0x00, 0xFF, 0x51, 0x03, 0x07, 0xA1, 0x20);

// Helper: variable length delta-time for 30 ticks (簡便)
...
const data = Buffer.from([...header, ...trackHeader, ...events]);
writeFileSync('data/export/cc64_sweep_ch1.mid', data);

console.log('✅ 生成: data/export/cc64_sweep_ch1.mid');
console.log(`イベント数: 上昇128 + 降下128 + テンポ + EOT = ${128+128+2}`);
