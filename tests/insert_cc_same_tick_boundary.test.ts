...
describe("insert_cc - same tick boundary", () => {
  it("startTick===endTick でもON/OFFが同tickに挿入（controller=1）", async () => {
    ...
    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "cc_same_tick.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_cc", arguments: { fileId, controller: 1, ranges: [ { startTick: 240, endTick: 240, valueOn: 70, valueOff: 20 } ] } } });
    ...
    const evs = body3.json.tracks.flatMap((t:any)=> t.events||[]).filter((e:any)=> e.type==="cc" && e.controller===1 && e.tick===240);
    const hasOn = evs.some((e:any)=> e.value===70);
    const hasOff = evs.some((e:any)=> e.value===20);
