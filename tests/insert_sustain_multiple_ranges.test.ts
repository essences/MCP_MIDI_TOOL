...
describe("insert_sustain - multiple ranges", () => {
  it("2レンジにCC64 ON/OFFを挿入", async () => {
    ...
    const base = {
      ...
      tracks: [
        ...
        { channel: 0, events: [ { type: "program", tick: 0, program: 0 }, { type: "note", tick: 0, pitch: 60, velocity: 100, duration: 1920 } ] }
      ]
    };

    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "pedal_multi.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    // 2つのレンジ: [0,480], [960,1440]
    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_sustain", arguments: { fileId, ranges: [ { startTick: 0, endTick: 480 }, { startTick: 960, endTick: 1440 } ] } } });
    ...
    const ons = new Set<number>();
    const offs = new Set<number>();
    for (const tr of tracks) for (const ev of tr.events || []) {
      if (ev.type === "cc" && ev.controller === 64 && ev.value === 127) ons.add(ev.tick);
      if (ev.type === "cc" && ev.controller === 64 && ev.value === 0) offs.add(ev.tick);
    }
    expect(ons.has(0)).toBe(true);
    expect(offs.has(480)).toBe(true);
    expect(ons.has(960)).toBe(true);
    expect(offs.has(1440)).toBe(true);
