...
describe("insert_cc - external channel mapping (1-16)", () => {
  it("channel: 6 (external) → internal 5 でCC11が入る", async () => {
    ...
    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "cc_ch_map.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    // 外部表記 channel=6 を指定（内部5にマップ）
    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_cc", arguments: { fileId, controller: 11, ranges: [ { startTick: 0, endTick: 480, channel: 6, valueOn: 90, valueOff: 40 } ] } } });
    ...
    const cc11 = tracks.flatMap(t => (t.events||[])).filter((e:any)=> e.type==="cc" && e.controller===11 && e.tick===0 && e.value===90);
    const ok = cc11.some((e:any)=> e.channel === 5);
    expect(ok).toBe(true);
