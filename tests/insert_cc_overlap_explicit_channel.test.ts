...
describe("insert_cc - overlap and explicit channel/track", () => {
  it("重なりレンジを順当結合し、明示channel/trackに挿入（controller=11）", async () => {
    ...
    // compile
    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "cc_overlap.mid", overwrite: true } } });
    ...
    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_cc", arguments: { fileId, controller: 11, ranges: [ { startTick: 0, endTick: 600, trackIndex: 2, channel: 5, valueOn: 90, valueOff: 40 }, { startTick: 480, endTick: 1200, trackIndex: 2, channel: 5, valueOn: 90, valueOff: 40 } ] } } });
    ...
    // json roundtrip and assert CC11 on track 2
    ...
    let cc11 = events.filter(e => e.type === "cc" && e.controller === 11);
    if (cc11.length === 0) {
      const all: any[] = [];
      for (const t of tracks) for (const e of (t.events||[])) if (e.type === "cc" && e.controller === 11) all.push(e);
      cc11 = all;
    }
    const have = (tick: number, value: number) => cc11.some(e => e.tick === tick && e.value === value);
    expect(have(0,90)).toBe(true);
    expect(have(600,40)).toBe(true);
    expect(have(480,90)).toBe(true);
    expect(have(1200,40)).toBe(true);
