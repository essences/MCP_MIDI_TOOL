...
describe("insert_sustain - external channel mapping (1-16)", () => {
  it("channel: 6 (external) → internal 5 でCC64が入る", async () => {
    ...
    const base = {
      ...
      tracks: [
        ...
        { channel: 5, events: [ { type: "program", tick: 0, program: 0 }, { type: "note", tick: 0, pitch: 60, velocity: 100, duration: 960 } ] }
      ]
    };

    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "pedal_ch_map.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    // 外部表記 channel=6 を指定（内部5にマップされる想定）
    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_sustain", arguments: { fileId, ranges: [ { startTick: 0, endTick: 480, channel: 6 } ] } } });
    ...
    const cc64 = tracks.flatMap(t => (t.events||[])).filter((e:any)=> e.type==="cc" && e.controller===64 && e.tick===0 && e.value===127);
    // 記録されたchannelが 5 であること（内部値）
    const ok = cc64.some((e:any)=> e.channel === 5);
    expect(ok).toBe(true);
