...
describe("insert_sustain - same tick boundary", () => {
  it("startTick===endTick でもON/OFFが同tickに挿入される", async () => {
    ...
    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "pedal_same_tick.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    // 同tickにON→OFF
    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_sustain", arguments: { fileId, ranges: [ { startTick: 240, endTick: 240 } ] } } });
    ...
    const evs = body3.json.tracks.flatMap((t:any)=> t.events||[]).filter((e:any)=> e.type==="cc" && e.controller===64 && e.tick===240);
