...
describe("insert_cc - basic", () => {
  it("controller=11(Expression) を範囲ON/OFFで挿入", async () => {
    ...
    sendLine(child, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "json_to_smf", arguments: { json: base, format: "json_midi_v1", name: "cc_basic.mid", overwrite: true } } });
    const res1 = await readLine(child);
    const body1 = JSON.parse(res1.result.content[0].text);
    const fileId = body1.fileId as string;

    sendLine(child, { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "insert_cc", arguments: { fileId, controller: 11, ranges: [ { startTick: 0, endTick: 480, valueOn: 90, valueOff: 40 } ] } } });
    ...
    const hasOn = evs.some((e:any)=> e.tick===0 && e.value===90);
    const hasOff = evs.some((e:any)=> e.tick===480 && e.value===40);
