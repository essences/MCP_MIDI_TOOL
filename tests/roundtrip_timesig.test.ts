...
describe("roundtrip timeSignature", () => {
  it("JSON→SMF→JSON でtimeSignatureが先頭トラックに現れる", async () => {
    ...
    const song = {
      ...
      ppq: 480,
      tracks: [
        { events: [ { type: "meta.tempo", tick: 0, usPerQuarter: 500000 }, { type: "meta.timeSignature", tick: 0, numerator: 3, denominator: 4 }, { type: "note", tick: 0, pitch: 60, velocity: 100, duration: 120 } ] }
      ]
    };
    ...
    expect(types).toContain("meta.timeSignature");
