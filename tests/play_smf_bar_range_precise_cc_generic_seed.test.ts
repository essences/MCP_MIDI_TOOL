...
/*
  REDテスト: 精密抽出 bar2..2 で 範囲前 (bar1) に存在した 任意 CC (controller 11: Expression) の値 45 が 0tick に再シードされること。
  現状は CC64 のみシード実装のため 失敗 (extracted.track0.events に controller=11 が無い) を期待。
*/
...
const JSON_MIDI = {
  ...
  tracks: [
    { events: [
      ...
      { type: 'meta.tempo', tick: 1920, usPerQuarter: 400000 }, // tempo change to enable precise gating
    ]},
    { channel:0, program:0, events: [
      { type: 'cc', tick: 0, controller: 11, value: 45 },
...
describe('play_smf precise bar extraction generic CC state seeding (RED)', () => {
  it('bar2 抽出で CC11=45 が 0tick に再シードされる (現状FAIL想定)', async () => {
    ...
    await recv(child);
    send(child,{jsonrpc:'2.0',id:2,method:'tools/call',params:{name:'json_to_smf',arguments:{ json: JSON_MIDI, format:'json_midi_v1', name:'cc11_seed.mid', overwrite:true }}});
    ...
    const hasSeed = track0Evts.some((e:any)=> e.type==='cc' && e.controller===11 && e.value===45 && e.tick===0);
    expect(hasSeed).toBe(true); // RED: ここで失敗するはず（まだ未実装）
    child.kill();
  }, 20000);
});
