...
let MidiOutput = null;
let MidiInput = null;
let lastMidiLoadError = null; // ネイティブ import 失敗内容を保持（後段 diagnostics で可視化）
...
async function main() {
    ...
    const warmup = { manifest: {}, schema: {}, midi: {} };
    const transport = new StdioServerTransport();
    const server = new Server({ name: "mcp-midi-tool", version: "0.1.0" }, 
    ...
    // 1. manifest スキャン（件数と時間）
    {
        ...
        let count = 0;
        try {
            count = (await readManifest()).items.length;
        }
        catch { /* ignore */ }
    ...
    // 2. スキーマ / コンパイルパス ウォーム（極小DSLをコンパイル）
    {
        const t1 = performance.now();
        try {
            const dummyScore = { ppq: 480, meta: { timeSignature: { numerator: 4, denominator: 4 }, tempo: { bpm: 120 } }, tracks: [{ channel: 1, events: [] }] };
    ...
    server.fallbackRequestHandler = async (request) => {
        ...
        const classifyError = (tool, err) => {
            ...
            if (/validation failed|compile failed|json validation failed/.test(lower)) {
                ...
                code = "NOT_FOUND";
                if (lower.includes("fileid"))
                    hint = "有効な fileId を list_midi や json_to_smf の結果から指定してください";
            }
            else if (/required/.test(lower)) {
                code = "MISSING_PARAMETER";
                const m = rawMsg.match(/'(\w+)' is required/i);
                if (m)
                    hint = `パラメータ ${m[1]} を arguments に追加してください`;
            }
        ...
        if (request.method === "prompts/get") {
            ...
            if (name === "score_dsl_quick_ref") {
                ...
                return { prompt: { name, description: "Score DSL v1 クイックリファレンス", messages: [{ role: "user", content: [{ type: "text", text }] }] } };
            }
            if (name === "trigger_notes_test_v9") {
                ...
                return { prompt: { name, description: "trigger_notes v9 テスト要約", messages: [{ role: "user", content: [{ type: "text", text }] }] } };
            }
            if (name === "single_capture_test_v1") {
                ...
                return { prompt: { name, description: "single_capture v1 テスト手順", messages: [{ role: "user", content: [{ type: "text", text }] }] } };
            }
        ...
        try {
            ...
            if (name === "trigger_notes") {
                ...
                let channel; // 内部0-15
                let resolvedChannelExternal; // 外部1-16
                const warnings = [];
                if (rawChannelInput === undefined) {
                    ...
                    if (!Number.isFinite(num))
                        throw new Error("channel must be a number (1-16)");
                    if (num >= 1 && num <= 16) {
                        // 正常な外部表記
                ...
                for (const n of rawNotes) {
                    if (typeof n === 'number' && Number.isFinite(n))
                        notes.push(Math.max(0, Math.min(127, (n | 0) + transpose)));
                ...
                const registry = globalThis.__playbacks = globalThis.__playbacks || new Map();
                if (dryRun) {
                ...
                try {
                    ...
                    if (!OutCls) {
                        ...
                        let target = 0;
                        const pickByHint = (o, hint) => {
                            for (let i = 0; i < ports; i++) {
                                try {
                                    const nm = o.getPortName(i);
                                    if (String(nm).toLowerCase().includes(hint))
                                        return i;
                                }
                                catch { }
                            }
                        ...
                        out.openPort(target);
                        try {
                            portNameResolved = String(out.getPortName?.(target));
                        }
                        catch { }
                        // optional program change
                        if (Number.isFinite(program)) {
                            out.sendMessage([0xC0 | (channel & 0x0f), program & 0x7f]);
                        }
                        ...
                        const to = setTimeout(() => {
                            try {
                                ...
                                try {
                                    const reg = globalThis.__playbacks;
                                    const st = reg?.get(playbackId);
                                    if (st) {
                                        if (st.active && typeof st.active.clear === 'function')
                                            st.active.clear();
                                        st.done = true;
                                    }
                                }
                ...
                return wrap({ ok: true, playbackId, scheduledNotes: notes.length, durationMs, portName: portNameResolved, channel: resolvedChannelExternal, internalChannel: channel, warnings: warnings.length ? warnings : undefined });
            }
            // store_midi: save base64 to data/midi and update manifest
            if (name === "store_midi") {
                const base64 = args?.base64;
                const fileNameInput = args?.name;
                const MAX_BYTES = 10 * 1024 * 1024; // 10MB
                if (!base64)
                    throw new Error("'base64' is required for store_midi");
                ...
                return wrap({ ok: true, fileId, path: relPath, bytes, createdAt });
            }
            // json_to_smf: validate/compile based on explicit format (if provided), then SMF保存
            if (name === "json_to_smf") {
                let json = args?.json;
                const format = typeof args?.format === 'string' ? String(args.format) : undefined;
                    ...
                    try {
                        if (obj && typeof obj === 'object') {
                            if (obj.format === 1 && typeof obj.ppq === 'number' && Array.isArray(obj.tracks)) {
                                ...
                                for (const t of obj.tracks) {
                                    if (t && Array.isArray(t.events)) {
                                        if (t.events.some((ev) => typeof ev?.tick === 'number'))
                                            return 'json_midi_v1';
                                    }
                                }
                            }
                            ...
                            if (typeof obj.ppq === 'number' && Array.isArray(obj.tracks)) {
                                for (const t of obj.tracks) {
                                    if (t && Array.isArray(t.events)) {
                                        if (t.events.some((ev) => ev && ev.start && (ev.start.bar || ev.start.beat) && ev.duration))
                                            return 'score_dsl_v1';
                                    }
                                }
                            }
                ...
                if (format === 'json_midi_v1') {
                    ...
                    try {
                        ...
                        const zIssues = Array.isArray(e?.issues) ? e.issues : [];
                        const summary = zIssues.slice(0, 5).map((i, idx) => `${idx}(${i.path?.join?.('.') || '(root)'}:${i.message})`).join(' | ');
                        const detailLines = zIssues.map((i, idx) => {
                            const pathStr = i.path?.join?.('.') || '(root)';
                            const code = i.code || 'ZOD_ERROR';
                            const exp = i.expected !== undefined ? ` expected=${JSON.stringify(i.expected)}` : '';
                            const rec = i.received !== undefined ? ` received=${JSON.stringify(i.received)}` : '';
                        ...
                        if (zIssues.length) {
                            err2.issues = zIssues.map((i) => ({ path: i.path, message: i.message, code: i.code, expected: i.expected, received: i.received }));
                        }
                        throw err2;
                    }
                }
                    ...
                    try {
                            ...
                            try {
                                ...
                                for (const ln of lines) {
                                    if (ln.startsWith('#title:'))
                                        title = ln.slice(7).trim();
                                    else if (ln.startsWith('#tempo:'))
                                        tempo = Number(ln.slice(7).trim());
                                    else if (ln.startsWith('#time:')) {
                                        const m = ln.slice(6).trim().match(/(\d+)\/(\d+)/);
                                        if (m)
                                            timeSig = { numerator: Number(m[1]), denominator: Number(m[2]) };
                                    }
                                    else {
                                        const m = ln.match(/^(\w+):\s*(.+)$/);
                                        if (m) {
                                            trackSpecs.push({ name: m[1], tokens: m[2].split(/\s+/) });
                                        }
                                    }
                                }
                                const ppq = 480;
                                ...
                                for (const ts of trackSpecs) {
                                    ...
                                    while (i < ts.tokens.length) {
                                        ...
                                        if (!lenToken)
                                            break;
                                        const lenMap = { '8': Math.round(ppq / 2), '4': ppq, '2': ppq * 2, '1': ppq * 4 };
                                        ...
                                        if (m) {
                                            ...
                                            if (base === -1) {
                                                ...
                                                if (enh[k])
                                                    base = names.indexOf(enh[k]);
                                            }
                                            const oct = Number(m[3]);
                                            if (base >= 0 && Number.isFinite(oct))
                                                pitch = base + (oct + 1) * 12;
                                        }
                                        if (pitch !== undefined) {
                        ...
                        const issuesArr = Array.isArray(e?.issues) ? e.issues : [];
                        const summary = issuesArr.slice(0, 5).map((i, idx) => `${idx}(${i.path?.join?.('.') || '(root)'}:${i.message})`).join(' | ');
                        const detailLines = issuesArr.map((i, idx) => {
                            const pathStr = i.path?.join?.('.') || '(root)';
                            const code = i.code || 'ZOD_ERROR';
                            const exp = i.expected !== undefined ? ` expected=${JSON.stringify(i.expected)}` : '';
                            const rec = i.received !== undefined ? ` received=${JSON.stringify(i.received)}` : '';
                        ...
                        if (issuesArr.length)
                            err2.issues = issuesArr.map(i => ({ path: i.path, message: i.message, code: i.code, expected: i.expected, received: i.received }));
                        throw err2;
                    }
                }
                else {
                    ...
                    if (parsed.success) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                const z2 = e2?.issues?.map?.((i) => `${i.path?.join?.('.')}: ${i.message}`).join('; ');
                            ...
                            if (!dslCompileError) {
                                const extra = e?.issues?.map?.((i) => `${i.path?.join?.('.')}: ${i.message}`).join('; ') || e?.message || String(e);
                ...
                const eventCount = Array.isArray(song.tracks) ? song.tracks.reduce((a, t) => a + (Array.isArray(t.events) ? t.events.length : 0), 0) : 0;
                const record = { id: fileId, name: nameWithExt, path: relPath, bytes, createdAt };
                await appendItem(record);
                inMemoryIndex.set(fileId, record);
                return wrap({ ok: true, fileId, path: relPath, bytes, createdAt, trackCount, eventCount });
            }
            // smf_to_json: read SMF, parse with @tonejs/midi, convert to JSON schema
            if (name === "smf_to_json") {
                const fileId = args?.fileId;
                if (!fileId)
                    throw new Error("'fileId' is required for smf_to_json");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
                ...
                const eventCount = Array.isArray(json.tracks) ? json.tracks.reduce((a, t) => a + (Array.isArray(t.events) ? t.events.length : 0), 0) : 0;
                return wrap({ ok: true, json, bytes, trackCount, eventCount });
            }
            // clean_midi: 重複メタ/チャネル別トラック統合（新規ファイル生成）
            if (name === "clean_midi") {
                const fileId = args?.fileId;
                if (!fileId)
                    throw new Error("'fileId' is required for clean_midi");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
            ...
            if (name === "append_to_smf") {
                const fileId = args?.fileId;
                if (!fileId)
                    throw new Error("'fileId' is required for append_to_smf");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
                ...
                if (!chunk)
                    throw new Error("'json' is required for append_to_smf");
                ...
                if (format === 'json_midi_v1') {
                    ...
                    if (parsed.success)
                        ...
                        addSong = zSong.parse(compileScoreToJsonMidi(chunk));
                }
                ...
                const existingMeta = { key: undefined, time: undefined, tempo: undefined };
                for (const ev of (baseJson.tracks[0]?.events || [])) {
                    if (!existingMeta.key && ev.type === 'meta.keySignature')
                        existingMeta.key = { sf: ev.sf, mi: ev.mi };
                    if (!existingMeta.time && ev.type === 'meta.timeSignature')
                        existingMeta.time = { numerator: ev.numerator, denominator: ev.denominator };
                    if (!existingMeta.tempo && ev.type === 'meta.tempo')
                        existingMeta.tempo = { usPerQuarter: ev.usPerQuarter };
                    if (existingMeta.key && existingMeta.time && existingMeta.tempo)
                        break;
                }
                // 追記チャンク内のグローバルメタを検査
                if (!keepGlobalMeta) {
                        ...
                        tr.events = tr.events.filter((ev) => {
                            if (ev.type === 'meta.keySignature') {
                                if (existingMeta.key) {
                                    ...
                                    // 同一キーは重複除去
                                    if (existingMeta.key.sf === ev.sf && existingMeta.key.mi === ev.mi)
                                        return false;
                                }
                            }
                            if (ev.type === 'meta.timeSignature') {
                                if (existingMeta.time) {
                                    if (existingMeta.time.numerator === ev.numerator && existingMeta.time.denominator === ev.denominator)
                                        return false; // 重複除去
                                }
                            }
                            if (ev.type === 'meta.tempo') {
                                if (existingMeta.tempo) {
                                    if (existingMeta.tempo.usPerQuarter === ev.usPerQuarter)
                                        return false;
                                }
                            }
                            return true;
                ...
                const trackMapping = Array.isArray(args?.trackMapping) ? args.trackMapping.map((n) => Math.max(0, Number(n) || 0)) : undefined;
                // 既存末尾tickを計測
                const trackEndTicks = baseJson.tracks.map((tr) => {
                    let last = 0;
                    for (const ev of (tr.events || [])) {
                        if (typeof ev.tick === 'number') {
                            if (ev.type === 'note')
                                last = Math.max(last, ev.tick + (ev.duration || 0));
                            else
                ...
                if (preserveTrackStructure) {
                    ...
                    if (!Number.isFinite(tgt)) {
                        const cand = baseJson.tracks.findIndex((tr) => (tr.events || []).some((e) => e.type !== "meta.tempo" && e.type !== "meta.timeSignature" && e.type !== "meta.keySignature"));
                        tgt = (cand >= 0 ? cand : 0);
                    }
                    if (!baseJson.tracks[tgt])
                        baseJson.tracks[tgt] = { events: [] };
                    ...
                    for (let srcTrackIdx = 0; srcTrackIdx < addSong.tracks.length; srcTrackIdx++) {
                        ...
                        for (const ev of srcTrack.events) {
                            if (ev.type === 'meta.tempo' || ev.type === 'meta.timeSignature' || ev.type === 'meta.keySignature') {
                                ...
                                if (!baseJson.tracks[0])
                                    baseJson.tracks[0] = { events: [] };
                ...
                const maxPrevTick = Math.max(0, ...baseJson.tracks.flatMap((t) => t.events.map((e) => e.tick || 0)));
            ...
            const captureRegistry = globalThis.__singleCaptures = globalThis.__singleCaptures || new Map();
            const continuousRecordingRegistry = globalThis.__continuousRecordings = globalThis.__continuousRecordings || new Map();
            // 24時間自動削除クリーンアップ
            ...
            function finalizeCapture(st, reason = 'completed') {
                if (st.done)
                    return;
                st.done = true;
                st.reason = reason;
            ...
            function maybeAutoFinalize(st) {
                ...
                if (elapsedSinceStart >= st.maxWaitMs) {
                    ...
                    finalizeCapture(st, 'timeout');
                    return;
                }
                if (!st.originMs)
                    return; // まだ最初のon無しだが maxWaitMs は既に上で判定済
                // 全ノートoff & サイレンス経過
                ...
                if (!anyActive) {
                    if (st.finalizeOnRelease) {
                        ...
                        if ((elapsedSinceStart - lastOff) >= st.silenceMs)
                            finalizeCapture(st, 'completed');
                    }
                }
            }
            if (name === 'start_single_capture') {
                ...
                const id = randomUUID();
                const st = { id, startedAt: Date.now(), onsetWindowMs, silenceMs, maxWaitMs, notes: new Map(), done: false, finalizeOnRelease };
                captureRegistry.set(id, st);
                return wrap({ ok: true, captureId: id, onsetWindowMs, silenceMs, maxWaitMs, finalizeOnRelease });
            }
            // --- MIDI入力デバイス列挙 ---
            if (name === 'list_input_devices') {
                const diagnostics = { platform: process.platform };
                try {
                    const r = await loadMidi();
                    if (!MidiInput) {
                        diagnostics.nativeLoadFailed = r.error ? (r.error.code || r.error.message || 'unknown') : 'MidiInputClassUnavailable';
                        return wrap({ ok: true, devices: [], diagnostics });
                    }
                    ...
                    try {
                        if (typeof inp.closePort === 'function')
                            inp.closePort();
                    }
                    catch { }
                    return wrap({ ok: true, devices, diagnostics });
                    ...
                    return wrap({ ok: false, error: { tool: 'list_input_devices', message: diagnostics.unhandledError }, diagnostics });
                }
            }
            // --- デバイスからの単発キャプチャ開始 ---
            if (name === 'start_device_single_capture') {
                await loadMidi();
                if (!MidiInput)
                    throw new Error('node-midi not available for input');
                const onsetWindowMs = Number.isFinite(Number(args?.onsetWindowMs)) ? Math.max(10, Math.min(500, Number(args.onsetWindowMs))) : 80;
                const silenceMs = Number.isFinite(Number(args?.silenceMs)) ? Math.max(50, Math.min(2000, Number(args.silenceMs))) : 150;
                const maxWaitMs = Number.isFinite(Number(args?.maxWaitMs)) ? Math.max(200, Math.min(10000, Number(args.maxWaitMs))) : 3000;
                const finalizeOnRelease = !!args?.finalizeOnRelease;
                const reqPortName = args?.portName;
                // 列挙しターゲットポートを決定
                ...
                if (reqPortName) {
                    ...
                    if (found >= 0)
                        ...
                        throw new Error(`input port not found: ${reqPortName}`);
                }
                ...
                try {
                    if (typeof inp.ignoreTypes === 'function')
                        inp.ignoreTypes(false, false, false);
                }
                catch { }
                const id = randomUUID();
                const st = { id, startedAt: Date.now(), onsetWindowMs, silenceMs, maxWaitMs, notes: new Map(), done: false, inputInstance: inp, inputPortName: ports[index], finalizeOnRelease };
                captureRegistry.set(id, st);
                // メッセージハンドラ
                const handler = (delta, message) => {
                    try {
                        ...
                        if (type === 0x90 && velocity > 0) {
                            if (!st.originMs)
                                st.originMs = st.startedAt + at;
                            const within = (st.startedAt + at - (st.originMs)) <= st.onsetWindowMs;
                            if (within) {
                                if (!st.notes.has(note))
                                    st.notes.set(note, { onAt: at, velocity: Math.max(1, Math.min(127, velocity)) });
                            }
                            else {
                ...
                return wrap({ ok: true, captureId: id, portName: ports[index], onsetWindowMs, silenceMs, maxWaitMs, finalizeOnRelease, mode: 'device' });
            }
            if (name === 'feed_single_capture') {
                const captureId = args?.captureId;
                const events = args?.events;
                if (!captureId)
                    throw new Error("'captureId' is required for feed_single_capture");
                if (!Array.isArray(events) || events.length === 0)
                    throw new Error("'events' must be non-empty array");
                const st = captureRegistry.get(captureId);
                if (!st)
                    throw new Error(`captureId not found: ${captureId}`);
                if (st.done)
                    return wrap({ ok: true, captureId, done: true, ignored: true });
                ...
                for (const ev of events) {
                    ...
                    if (!Number.isFinite(note) || note < 0 || note > 127)
                        throw new Error(`invalid note: ${ev?.note}`);
                    if (!Number.isFinite(at) || at < 0)
                        throw new Error(`invalid at ms: ${ev?.at}`);
                    ...
                    if (kind === 'on') {
                        ...
                        if (within) {
                            if (!st.notes.has(note))
                                st.notes.set(note, { onAt: at, velocity: Number.isFinite(velRaw) ? Math.max(1, Math.min(127, velRaw)) : 100 });
                        }
                        else {
                ...
                // 自動完了判定
                maybeAutoFinalize(st);
                return wrap({ ok: true, captureId, done: st.done });
            }
            if (name === 'get_single_capture_status') {
                const captureId = args?.captureId;
                if (!captureId)
                    throw new Error("'captureId' is required for get_single_capture_status");
                const st = captureRegistry.get(captureId);
                if (!st)
                    throw new Error(`captureId not found: ${captureId}`);
                maybeAutoFinalize(st);
                if (st.done && !st.result)
                    finalizeCapture(st, st.reason || 'completed');
                return wrap({ ok: true, captureId, done: st.done, reason: st.reason, result: st.result });
            }
            // --- 継続記録ツール実装 ---------------------------------------------
            // 継続記録セッション管理関数
            function createContinuousSession(portName, ppq, maxDurationMs, idleTimeoutMs, silenceTimeoutMs, channelFilter, eventTypeFilter) {
                const id = randomUUID();
                const session = {
                    id,
                    startedAt: Date.now(),
            ...
            // 継続記録セッションをSMFとして保存
            async function saveContinuousRecordingAsSmf(session, name, overwrite) {
                // ファイル名生成・重複回避
                ...
                const jsonMidi = {
                    ...
                    tracks: [
                        {
                            events: session.events // 型チェックをスキップ（実行時は正しいイベント形式）
                        }
                    ]
                };
            ...
            function msToTick(relativeMs, ppq) {
            ...
            // 小節計算ヘルパー関数
            function barToTick(bar, ppq, timeSignature = { numerator: 4, denominator: 4 }) {
            ...
            function addEventToSession(session, midiBytes, relativeMs) {
            ...
            if (name === 'start_continuous_recording') {
                ...
                const activeSessions = Array.from(continuousRecordingRegistry.values()).filter(s => s.status === 'waiting_for_input' || s.status === 'recording');
                ...
                const reqPortName = args?.portName;
                const channelFilter = Array.isArray(args?.channelFilter) ? args.channelFilter.filter((n) => Number.isInteger(n) && n >= 1 && n <= 16) : undefined;
                const eventTypeFilter = Array.isArray(args?.eventTypeFilter) ? args.eventTypeFilter.filter((s) => typeof s === 'string' && ['note', 'cc', 'pitchBend', 'program'].includes(s)) : undefined;
                // 入力ポート決定
                const temp = new MidiInput();
                const count = typeof temp.getPortCount === 'function' ? temp.getPortCount() : 0;
                const ports = [];
                ...
                // セッション作成
                const session = createContinuousSession(actualPortName, ppq, maxDurationMs, idleTimeoutMs, silenceTimeoutMs, channelFilter, eventTypeFilter);
                // MIDI入力設定
            ...
            if (name === 'get_continuous_recording_status') {
                ...
                if (!recordingId)
                    throw new Error("'recordingId' is required for get_continuous_recording_status");
            ...
            if (name === 'stop_continuous_recording') {
                ...
                if (!recordingId)
                    throw new Error("'recordingId' is required for stop_continuous_recording");
                const session = continuousRecordingRegistry.get(recordingId);
                if (!session)
                    throw new Error(`recording session not found: ${recordingId}`);
                // 手動終了の場合、セッション状態を更新
            ...
            if (name === 'list_continuous_recordings') {
                ...
                if (statusFilter === 'active') {
                    filteredSessions = allSessions.filter(s => s.status === 'waiting_for_input' || s.status === 'recording');
                }
                ...
                // 統計情報
                const activeCount = allSessions.filter(s => s.status === 'waiting_for_input' || s.status === 'recording').length;
                const completedCount = allSessions.filter(s => s.status === 'completed' ||
                    s.status === 'timeout_idle' ||
                    s.status === 'timeout_silence' ||
                    s.status === 'timeout_max_duration' ||
                    s.status === 'stopped_manually').length;
                return wrap({
                    ok: true,
                    recordings,
            ...
            if (name === "insert_sustain") {
                ...
                if (!fileId)
                    throw new Error("'fileId' is required for insert_sustain");
                ...
                const pickTrackIndex = () => {
                    // 音源イベントのある最初のトラック
                    const cand = json.tracks.findIndex((tr) => (tr.events || []).some((e) => e.type !== 'meta.tempo' && e.type !== 'meta.timeSignature' && e.type !== 'meta.keySignature'));
                    return cand >= 0 ? cand : 0;
                };
                const ensureTrack = (idx) => { if (!json.tracks[idx])
                    json.tracks[idx] = { events: [] }; };
                const guessChannelFromTrack = (tr) => {
                    if (Number.isFinite(Number(tr?.channel)))
                        return (tr.channel | 0);
                    const ev = (tr?.events || []).find((e) => (e.channel !== undefined));
                    if (ev && Number.isFinite(Number(ev.channel)))
                        return (ev.channel | 0);
                    return undefined;
                };
                for (const r of ranges) {
                    const start = Math.max(0, Number(r.startTick | 0));
                    const end = Math.max(start, Number(r.endTick | 0));
                    const rr = r;
                    ...
                    const removeExisting = rr.removeExisting !== false; // 既定 true
                    // 挿入先
                    ...
                    let chVal = Number.isFinite(Number(rr.channel)) ? Number(rr.channel) : guessChannelFromTrack(json.tracks[tIdx]);
                    let ch;
                    ...
                    if (removeExisting) {
                        json.tracks[tIdx].events = (json.tracks[tIdx].events || []).filter((ev) => {
                            if (ev?.type !== 'cc')
                                return true;
                            if ((ev.controller | 0) !== 64)
                                return true;
                    ...
                    json.tracks[tIdx].events = json.tracks[tIdx].events.filter((ev, i, arr) => {
                        if (ev.type !== 'cc' || ev.controller !== 64)
                            return true;
                        const key = `${ev.tick}:${ev.value}:${ev.channel ?? 'x'}`;
                        const first = arr.findIndex((e) => e.type === 'cc' && e.controller === 64 && (e.tick | 0) === (ev.tick | 0) && (e.value | 0) === (ev.value | 0) && ((e.channel ?? 'x') === (ev.channel ?? 'x')));
                        return first === i;
                ...
                await writeManifest(manifest);
                inMemoryIndex.set(item.id, { ...item, bytes, path: relPath });
                return wrap({ ok: true, fileId: item.id, name: item.name, path: relPath, bytes });
            }
            ...
            if (name === "insert_cc") {
                ...
                const ranges = args?.ranges;
                if (!fileId)
                    throw new Error("'fileId' is required for insert_cc");
                if (!Number.isFinite(Number(controller)))
                    throw new Error("'controller' is required (0-127)");
                ...
                for (const r of ranges) {
                    ...
                    json.tracks[tIdx].events = json.tracks[tIdx].events.filter((ev, i, arr) => {
                        if (ev.type !== 'cc' || ev.controller !== ctrl)
                            return true;
                        const key = `${ev.tick}:${ev.value}:${ev.channel ?? 'x'}`;
                        const first = arr.findIndex((e) => e.type === 'cc' && e.controller === ctrl && (e.tick | 0) === (ev.tick | 0) && (e.value | 0) === (ev.value | 0) && ((e.channel ?? 'x') === (ev.channel ?? 'x')));
                        return first === i;
                    }).sort((a, b) => (a.tick | 0) - (b.tick | 0));
                }
                // 再エンコードして保存（上書き）
            ...
            if (name === "extract_bars") {
                const fileId = args?.fileId;
                const startBar = args?.startBar;
                const endBar = args?.endBar;
                const format = args?.format || "json_midi_v1";
                if (!fileId)
                    throw new Error("'fileId' is required for extract_bars");
                if (!Number.isFinite(Number(startBar)) || Number(startBar) < 1)
                    throw new Error("'startBar' must be >= 1");
                if (!Number.isFinite(Number(endBar)) || Number(endBar) < 1)
                    throw new Error("'endBar' must be >= 1");
                if (Number(startBar) > Number(endBar))
                    throw new Error("'startBar' must be <= 'endBar'");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
                ...
                return wrap({
                    ...
                    ppq,
                    timeSignature,
                    eventCount: extractedJson.tracks.reduce((sum, track) => sum + (track.events?.length || 0), 0),
            ...
            if (name === "replace_bars") {
                ...
                if (!fileId)
                    throw new Error("'fileId' is required for replace_bars");
                if (!Number.isFinite(Number(startBar)) || Number(startBar) < 1)
                    throw new Error("'startBar' must be >= 1");
                if (!Number.isFinite(Number(endBar)) || Number(endBar) < 1)
                    throw new Error("'endBar' must be >= 1");
                if (Number(startBar) > Number(endBar))
                    throw new Error("'startBar' must be <= 'endBar'");
                if (!jsonData)
                    throw new Error("'json' is required for replace_bars");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
                ...
                const modifiedJson = {
                    ...originalJson,
                    tracks: originalJson.tracks.map((track, trackIndex) => {
                        // 指定範囲外のイベントを保持
                        ...
                        if (trackIndex === 0) {
                            ...
                            for (const replacementTrack of (replacementJson.tracks || [])) {
                                const trackEvents = (replacementTrack.events || [])
                                    .filter((event) => !event.type?.startsWith('meta.')) // メタイベント除外
                                    .map((event) => ({
                                    ...event,
                        ...
                        return {
                            ...track,
                            events: [...eventsOutsideRange, ...replacementEvents].sort((a, b) => (a.tick || 0) - (b.tick || 0))
                        };
                    })
                };
                // SMFファイルとして保存
                ...
                return wrap({
                    ...
                    startTick,
                    endTick,
                    replacedEventCount: replacementJson.tracks?.reduce((sum, track) => sum + (track.events?.length || 0), 0) || 0,
                    totalEventCount: modifiedJson.tracks.reduce((sum, track) => sum + (track.events?.length || 0), 0)
                });
            }
            // play_smf: parse SMF and schedule playback (or dryRun for analysis only)
            if (name === "play_smf") {
                const fileId = args?.fileId;
                const startMs = Number.isFinite(Number(args?.startMs)) ? Number(args?.startMs) : undefined;
                const stopMs = Number.isFinite(Number(args?.stopMs)) ? Number(args?.stopMs) : undefined;
                // 新規: 小節範囲指定（ms指定が存在する場合は優先される）
                const startBar = Number.isFinite(Number(args?.startBar)) ? Math.max(1, Math.floor(Number(args?.startBar))) : undefined;
                const endBar = Number.isFinite(Number(args?.endBar)) ? Math.max(1, Math.floor(Number(args?.endBar))) : undefined;
                const dryRun = !!args?.dryRun;
                const schedulerLookaheadMs = Number.isFinite(Number(args?.schedulerLookaheadMs)) ? Math.max(10, Math.min(1000, Number(args?.schedulerLookaheadMs))) : undefined;
                const schedulerTickMs = Number.isFinite(Number(args?.schedulerTickMs)) ? Math.max(5, Math.min(200, Number(args?.schedulerTickMs))) : undefined;
                if (!fileId)
                    throw new Error("'fileId' is required for play_smf");
                ...
                try {
                    ...
                    if (!Midi)
                        throw new Error('Midi class not found in @tonejs/midi');
                    const midi = new Midi(buf);
                    // --- 精密小節抽出モード: ms指定なし かつ bar指定あり かつ env が simple でない場合 ---
                    const hasTempoChange = Array.isArray(midi.header?.tempos) && midi.header.tempos.length > 1;
                    const hasTimeSigChange = Array.isArray(midi.header?.timeSignatures) && midi.header.timeSignatures.length > 1;
                    ...
                    if (wantPreciseBar) {
                        try {
                            ...
                            const baseJson = await (async () => {
                                ...
                                const fn = mod.decodeSmfToJson || mod.default?.decodeSmfToJson || mod.default || mod.smfToJson;
                                if (typeof fn !== 'function')
                                    throw new Error('decodeSmfToJson function not found');
                            ...
                            const timeSigs = baseJson.tracks.flatMap((t) => (t.events || []).filter((e) => e.type === 'meta.timeSignature')).sort((a, b) => a.tick - b.tick);
                            const tempoEvts = baseJson.tracks.flatMap((t) => (t.events || []).filter((e) => e.type === 'meta.tempo')).sort((a, b) => a.tick - b.tick);
                            const getEffectiveTS = (tick) => {
                                let cur = { numerator: 4, denominator: 4 };
                                for (const ts of timeSigs) {
                                    if (ts.tick <= tick)
                                        cur = { numerator: ts.numerator, denominator: ts.denominator };
                                    else
                                        break;
                                }
                                return cur;
                            ...
                            // 近似: barToTick を逐次生成
                            const barToTickCache = new Map();
                            function barToTick(bar) {
                                if (barToTickCache.has(bar))
                                    return barToTickCache.get(bar);
                                if (bar === 1) {
                                    barToTickCache.set(1, 0);
                                    return 0;
                                }
                                // 前のバー末尾tick + 1バー長
                            ...
                            for (const tr of baseJson.tracks) {
                                ...
                                for (const ev of (tr.events || [])) {
                                    ...
                                    if (typeof tk === 'number') {
                                        ...
                                        if (shifted.type === 'note')
                                            shifted.duration = Math.max(1, Math.min(shifted.duration, endTickExclusive - tk));
                                        evs.push(shifted);
                                    }
                                }
                                if (evs.length)
                                    newTracks.push({ ...tr, events: evs });
                            }
                            // track0 メタ再シード: 最新 tempo/timeSig/keySig を 0tick に配置
                            const seedMeta = [];
                            const latest = (list, pred) => {
                                let sel;
                                for (const e of list) {
                                    if (e.tick <= startTick && pred(e))
                                        sel = e;
                            ...
                            const latestTS = latest(timeSigs, () => true);
                            if (latestTS)
                                seedMeta.push({ type: 'meta.timeSignature', tick: 0, numerator: latestTS.numerator, denominator: latestTS.denominator });
                            const keySigs = baseJson.tracks.flatMap((t) => (t.events || []).filter((e) => e.type === 'meta.keySignature')).sort((a, b) => a.tick - b.tick);
                            const latestKS = latest(keySigs, () => true);
                            if (latestKS)
                                seedMeta.push({ type: 'meta.keySignature', tick: 0, sf: latestKS.sf, mi: latestKS.mi });
                            const latestTempo = latest(tempoEvts, () => true);
                            if (latestTempo)
                                seedMeta.push({ type: 'meta.tempo', tick: 0, usPerQuarter: latestTempo.usPerQuarter });
                            ...
                            try {
                                const ccEventsAll = baseJson.tracks.flatMap((t) => (t.events || []).filter((e) => e.type === 'cc' && typeof e.controller === 'number'));
                                // controller × channel の最新を抽出 (tick <= startTick)
                                const latestPerKey = new Map();
                                for (const ev of ccEventsAll.sort((a, b) => (a.tick | 0) - (b.tick | 0))) {
                                ...
                                try {
                                    const pbEventsAll = baseJson.tracks.flatMap((t) => (t.events || []).filter((e) => e.type === 'pitchBend'));
                                    if (process.env.MCP_MIDI_PLAY_SMF_DEBUG_JSON) {
                                        // デバッグ: startTick 以前/以後の pitchBend 一覧
                                        const dbgBefore = pbEventsAll.filter((e) => (e.tick | 0) <= startTick).map((e) => ({ tick: e.tick, value: e.value, channel: e.channel }));
                                        const dbgAfter = pbEventsAll.filter((e) => (e.tick | 0) > startTick).map((e) => ({ tick: e.tick, value: e.value, channel: e.channel }));
                                        track0.events.push({ type: 'meta.marker', tick: 0, text: `DBG_PB<= ${JSON.stringify(dbgBefore).slice(0, 200)}` });
                                        track0.events.push({ type: 'meta.marker', tick: 0, text: `DBG_PB> ${JSON.stringify(dbgAfter).slice(0, 200)}` });
                                    }
                                    const latestPBPerCh = new Map();
                                    for (const ev of pbEventsAll.sort((a, b) => (a.tick | 0) - (b.tick | 0))) {
                                        if ((ev.tick | 0) <= startTick) {
                                            const ch = Number.isFinite(Number(ev.channel)) ? (ev.channel | 0) : 0;
                            ...
                            const encFn = mod2.encodeToSmfBinary || mod2.default?.encodeToSmfBinary || mod2.default;
                            if (typeof encFn !== 'function')
                                throw new Error('encodeToSmfBinary function not found');
                            const smfBin = encFn(extracted);
                            // 再度 Midi に読み込み直し
                            ...
                            midi._tracks = midi2.tracks; // 内部差し替え（簡易）
                    ...
                    for (const tr of midi.tracks) {
                        ...
                        for (const nt of (tr?.notes || [])) {
                            ...
                            if (stopMs !== undefined && tOnMs > stopMs)
                                continue;
                            if (startMs !== undefined && tOffMs < startMs)
                                continue;
                    ...
                    if (startMs !== undefined || stopMs !== undefined) {
                        ...
                        try {
                            if (Array.isArray(header.timeSignatures) && header.timeSignatures.length) {
                                const ts0 = header.timeSignatures[0];
                                if (Array.isArray(ts0.timeSignature)) {
                                    num = ts0.timeSignature[0] || 4;
                                    den = ts0.timeSignature[1] || 4;
                                }
                                else if (typeof ts0.numerator === 'number' && typeof ts0.denominator === 'number') {
                                    num = ts0.numerator;
                                    den = ts0.denominator;
                                }
                            }
                ...
                if (dryRun || events.length === 0) {
                    ...
                    return wrap({ ok: true, playbackId, scheduledEvents, totalDurationMs, extractionMode, ...(debugExtracted ? { debug: { extracted: debugExtracted } } : {}), warnings: warnings.length ? warnings : undefined });
                }
                // 実再生: ルックアヘッドスケジューラ
                const state = {
                ...
                try {
                    ...
                    if (OutCls) {
                        ...
                        const pickByHint = (hint) => {
                            for (let i = 0; i < ports; i++) {
                                try {
                                    ...
                                    if (String(name).toLowerCase().includes(hint))
                                        return i;
                                }
                                catch { }
                            }
                        ...
                        try {
                            state.portName = String(out.getPortName?.(target));
                        }
                        catch { }
                    }
                    else {
                    ...
                    if (cursor >= events.length) {
                        ...
                        try {
                            if (state.out) {
                                const chSet = new Set();
                                for (const ev of events)
                                    chSet.add((ev.ch | 0) & 0x0f);
                                for (const ch of chSet) {
                                    // CC64 (Sustain) OFF
                                    try {
                                        state.out.sendMessage([0xB0 | ch, 64, 0]);
                                    }
                                    catch { }
                                }
                                // 残留ノートがあれば個別にOff
                                if (state.active && state.active.size) {
                                    try {
                                        const chSet2 = new Set();
                                        for (const key of Array.from(state.active)) {
                                            const [chStr, nStr] = String(key).split(":");
                                            const ch = (Number(chStr) | 0) & 0x0f;
                                            const n = Number(nStr) | 0;
                                            chSet2.add(ch);
                                            try {
                                                state.out.sendMessage([0x90 | ch, n & 0x7f, 0]);
                                            }
                                            catch { }
                                        }
                                        // CC123 All Notes Off
                                        for (const ch of chSet2) {
                                            try {
                                                state.out.sendMessage([0xB0 | ch, 123, 0]);
                                            }
                                            catch { }
                                        }
                                        state.active.clear();
                ...
                return wrap({ ok: true, playbackId, scheduledEvents, totalDurationMs, warnings: warnings.length ? warnings : undefined });
            }
            // get_midi: retrieve file metadata and optionally base64 content
            if (name === "get_midi") {
                const fileId = args?.fileId;
                const includeBase64 = !!args?.includeBase64;
                if (!fileId)
                    throw new Error("'fileId' is required for get_midi");
                let item = inMemoryIndex.get(fileId);
                if (!item)
                    item = (await getItemById(fileId));
                if (!item)
            ...
            if (name === "list_midi") {
                ...
                let items = [];
                try {
                    items = (await readManifest()).items;
                }
                catch {
                    items = [];
                }
                ...
                return wrap({ ok: true, total, items: slice });
            }
            // export_midi: copy file to data/export directory
            if (name === "export_midi") {
                const fileId = args?.fileId;
                if (!fileId)
                    throw new Error("'fileId' is required for export_midi");
                ...
                return wrap({ ok: true, exportPath });
            }
                ...
                if (process.platform === "darwin") {
                    ...
                    if (nativeLoadFailed) {
                        ...
                        if (lastMidiLoadError) {
                            ...
                            if (msg && !diagnostics.nativeLoadErrorMessage)
                                diagnostics.nativeLoadErrorMessage = msg.slice(0, 400);
                        }
                    }
            ...
            if (name === "playback_midi") {
                ...
                if (!fileId)
                    throw new Error("'fileId' is required for playback_midi");
                ...
                // メモリ状態に記録
                globalThis.__playbacks = globalThis.__playbacks || new Map();
                globalThis.__playbacks.set(playbackId, { fileId, portName: portName || null, startedAt: Date.now() });
                return wrap({ ok: true, playbackId });
            }
            // stop_playback: stop a running playback (stubbed)
            if (name === "stop_playback") {
                const playbackId = args?.playbackId;
                if (!playbackId)
                    throw new Error("'playbackId' is required for stop_playback");
                const map = globalThis.__playbacks;
            ...
            if (name === "get_playback_status") {
                const playbackId = args?.playbackId;
                if (!playbackId)
                    throw new Error("'playbackId' is required for get_playback_status");
                const map = globalThis.__playbacks;
                const st = map?.get(playbackId);
                if (!st)
                    return wrap({ ok: false, error: 'not_found' });
                const now = performance.now();
    ...
    if (manifestItemsThresholdExceeded) {
        try {
            process.stderr.write(`[WARN] manifest item count high: ${manifestItems} >= ${manifestThreshold}\n`);
        }
        catch { /* ignore */ }
    }
