 ...
 */
export function cleanJsonMidiSong(song) {
    const canonicalTempo = [];
    const canonicalTS = [];
    const canonicalKS = [];
    let removedMeta = 0;
    ...
    for (const tr of song.tracks) {
        tr.events = tr.events.filter(ev => {
            ...
            if (ev.type === 'meta.tempo') {
                if (canonicalTempo.length === 0) {
                    canonicalTempo.push(ev);
                    return false;
                } // pull out into canonical set
                removedMeta++;
                return false;
            }
    ...
    for (const ev of [...canonicalTempo, ...canonicalTS, ...canonicalKS]) {
        ev.tick = 0;
    }
    const track0 = { events: [...canonicalTempo, ...canonicalTS, ...canonicalKS, ...orphanEvents] };
    // Remove any accidental duplicate meta of same type at same tick that slipped in via orphanEvents
    const seenMetaTypes = new Set();
    track0.events = track0.events.filter((ev) => {
        if (ev?.type?.startsWith('meta.')) {
            if (['meta.tempo', 'meta.timeSignature', 'meta.keySignature'].includes(ev.type)) {
                if (seenMetaTypes.has(ev.type)) {
                    removedMeta++;
                    return false;
                }
    ...
    track0.events.sort((a, b) => (a.tick || 0) - (b.tick || 0));
    const newTracks = [track0, ...Array.from(byChannel.values())];
    const cleaned = { ppq: song.ppq, tracks: newTracks };
    return { cleaned, removedMeta, mergedTracks: song.tracks.length - mergedTracks };
}
export async function cleanMidiFile(absPath) {
    const data = await fs.readFile(absPath);
    const decoded = await decodeSmfToJson(data); // relaxed assertion for cleaning scope
    ...
    // Encode
    const smf = encodeToSmfBinary({ ppq: cleaned.ppq || 480, format: 1, tracks: cleaned.tracks });
